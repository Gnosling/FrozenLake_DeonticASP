...
def plan_action(level: str, current_state_of_traverser: int, last_performed_action: str, current_state: int = 0, norm_set: int = 1, evaluation_function: int = 1):
    """
    ...
    if DEBUG_MODE:
        clingo.clingo_main(tel, ['--quiet=1', '--opt-strategy=bb,hier', '--opt-mode=opt', '--time-limit=10', '--istop=sat', file1, file2, file3, file4, file5, file6])
    else:
        clingo.clingo_main(tel, ['--verbose=0', '--quiet=1,2,2', '--opt-strategy=bb,hier', '--opt-mode=opt', '--time-limit=10', '--istop=sat', file1, file2, file3, file4, file5, file6])
