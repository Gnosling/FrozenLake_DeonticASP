...
class PlannerPolicy(Policy):
    ...
    def __init__(self, q_table: QTable, learning_rate: float, discount: float, strategy: str, level: str, norm_set: int, evaluation_function: int):
    ...
    def suggest_action(self, state) -> Any:
        ...
        if self.strategy == "full_planning":
            action = plan_action(self.level, self.current_state_of_traverser, self.last_performed_action, state, self.norm_set, self.evaluation_function)
            ...
            # ToDo: activate planning based on epsilon?
            action = self.q_table.get_best_action_for_state(state)
